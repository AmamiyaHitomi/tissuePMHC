...
Performance after excluding each held-out peptide from all training combinations is reported once, together with the ordinary same-pool reference and uncertainty, under Research Question 4 (Tables~\ref{tab:human-standard-strict-summary} and~\ref{tab:mouse-standard-strict-summary}).
...
The same question is evaluated in mouse to determine whether the peptide-separation gap recurs in an independently constructed tissue--H2 benchmark. Both analyses evaluate the same 6,766 pairs. Among 72 combination-by-fold cells, 48 have identical held-out sizes and the remainder differ by one pair, giving a mean absolute difference of 0.333. In the ordinary folds, 71.43--72.12\% of unique held-out peptides reappear during fitting, compared with 0\% after peptide separation.

\begin{table}[tbp]
\centering
\caption{Mouse AUROC under same-pool ordinary cross-validation and cross-validation excluding each held-out peptide from all fitting combinations. Difference is peptide-separated minus ordinary same-pool cross-validation; a negative value denotes lower performance after exact peptide separation.}
...
The relationship across mouse tissue--H2 combinations is shown separately in Figure~\ref{fig:mouse-standard-strict-scatter}.
\begin{figure}[!t]
\centering
\begin{tikzpicture}
\begin{groupplot}[
...
\end{groupplot}
\end{tikzpicture}
\caption{Mouse AUROC for each tissue--H2 combination under ordinary same-pool and peptide-separated cross-validation. Points below the gray equality line have lower performance after exact-peptide separation.}
\label{fig:mouse-standard-strict-scatter}
\end{figure}

Mean mouse AUROC decreases by 0.0863 and is lower for 22 of 24 tissue--H2 combinations. The 95\% interval obtained by resampling complete combinations is -0.1106 to -0.0632, and within-pair ordering accuracy decreases by 0.0615. The numerically larger decrease should not be interpreted as a species effect because the datasets and models differ.

For both species, saved-test results are not subtracted from peptide-separated cross-validation results because the evaluations contain different examples.
...
The mouse benchmark asks whether the same MHC- and parent-protein-controlled comparison is learnable across 24 tissue--H2 combinations. A separate mouse model is used, and no human parameters are transferred.
...
Full multi-kernel TissuePMHC$^\dagger$ & 0.8528 & 0.8477 & 0.7196 & three-run aggregate \\
Selected Factorized MMoE$^\dagger$ & \textbf{0.8562} & \textbf{0.8506} & \textbf{0.7245} & five-run aggregate \\
\bottomrule
\end{tabularx}
\end{table*}

Table~\ref{tab:mouse-baselines} adds the completed mouse implementations of method families evaluated in the human benchmark. Every row uses the same fixed mouse training and test partitions, and every metric gives equal weight to the 24 combinations. The H2 pseudo-sequence-conditioned and ID--pseudo-sequence hybrid models are omitted because no provenance-verified H2 pseudo-sequence file was available. The selected Factorized MMoE remains numerically strongest, but the full multi-kernel TissuePMHC and MLP dual-branch aggregate are close. Because multiple architectures have now been compared retrospectively on the saved test set, this survey is an internal benchmark comparison rather than untouched confirmation or external validation.

The mouse architecture, training settings, repeated runs, and averaging rule used for the selected Factorized MMoE were specified before its evaluation. Table~\ref{tab:mouse-protocol-summary} places that model's result beside ordinary same-pool and harder peptide-separated cross-validation.

\begin{table*}[t]
\centering
\caption{Evaluation-protocol summary for the selected mouse Factorized MMoE. The saved-test row is an internal result in familiar tissue--H2 combinations; peptide-separated cross-validation evaluates peptide identities absent from all fitting combinations.}
...
\toprule
Evaluation & Prediction aggregation & AUROC & AUPRC & Worst-6 & \shortstack{Within-pair\\accuracy} \\ 
\midrule
Ordinary same-pool cross-validation & Average across five runs & 0.8392 & 0.8316 & 0.7101 & 0.8211 \\ 
Saved internal test set & Average across five prespecified runs & \textbf{0.8562} & \textbf{0.8506} & \textbf{0.7245} & \textbf{0.8425} \\ 
Peptide-separated cross-validation & Average across five runs & 0.7529 & 0.7322 & 0.6481 & 0.7596 \\ 
\bottomrule
\end{tabularx}
\end{table*}

For the selected model, the saved-test values are descriptively higher than ordinary same-pool cross-validation by 0.0170 AUROC, 0.0190 AUPRC, and 0.0144 for the worst-six mean. Eighteen of 24 combinations improve and six decline. Because the evaluated examples differ, substantial entity overlap remains, and additional architectures were subsequently evaluated on the same saved set (Table~\ref{tab:mouse-baselines}), this comparison is neither an estimate of peptide-separation effects nor independent validation.
...
Equal-weight summaries within each tissue also reveal marked heterogeneity. Table~\ref{tab:mouse-tissue-complete} reports all 13 tissues. Tissue means give equal weight to the available H-2 restrictions, and each tissue--H-2 combination contributes 100 saved-test pairs. Single-restriction tissues are descriptive and do not provide a within-tissue estimate of restriction-to-restriction variation.

\begin{table*}[!t]
\centering
\caption{Complete mouse tissue-level results for the selected five-run Factorized MMoE on the saved internal test set. AUROC, AUPRC, and within-pair accuracy are averaged with equal weight across the available H-2 restrictions. The range gives the minimum and maximum task AUROC within each tissue.}
\label{tab:mouse-tissue-complete}
\fontsize{7.5}{8.5}\selectfont
\setlength{\tabcolsep}{3pt}
\begin{tabularx}{\textwidth}{@{}>{\raggedright\arraybackslash}Xrrrrrr@{}}
\toprule
Tissue & \shortstack{H-2\\restrictions} & \shortstack{Test\\pairs} & AUROC & AUPRC & \shortstack{Within-pair\\accuracy} & \shortstack{Task AUROC\\range} \\
\midrule
Colon & 2 & 200 & 0.7059 & 0.7186 & 0.6950 & 0.6840--0.7277 \\
Pancreas & 1 & 100 & 0.7091 & 0.6737 & 0.6500 & 0.7091--0.7091 \\
Prostate & 1 & 100 & 0.7974 & 0.7719 & 0.7900 & 0.7974--0.7974 \\
Skin & 4 & 400 & 0.8180 & 0.8075 & 0.8175 & 0.7216--0.9197 \\
Spleen & 4 & 400 & 0.8353 & 0.8257 & 0.8325 & 0.7920--0.8787 \\
Liver & 4 & 400 & 0.8432 & 0.8566 & 0.8275 & 0.7639--0.8862 \\
Thymus & 2 & 200 & 0.9125 & 0.9020 & 0.8800 & 0.8430--0.9820 \\
Lymphoid & 1 & 100 & 0.9284 & 0.9044 & 0.9100 & 0.9284--0.9284 \\
Lung & 1 & 100 & 0.9641 & 0.9579 & 0.9800 & 0.9641--0.9641 \\
Bone marrow & 1 & 100 & 0.9734 & 0.9558 & 0.9300 & 0.9734--0.9734 \\
Small intestine & 1 & 100 & 0.9793 & 0.9748 & 0.9500 & 0.9793--0.9793 \\
Kidney & 1 & 100 & 0.9837 & 0.9844 & 0.9700 & 0.9837--0.9837 \\
Uterus & 1 & 100 & 0.9910 & 0.9916 & 0.9800 & 0.9910--0.9910 \\
\bottomrule
\end{tabularx}
\end{table*}

When every test peptide is absent from mouse fitting data, AUROC is 0.7529, with a 95\% combination-resampling interval of 0.7230--0.7799. AUPRC is 0.7322, and 75.96\% of matched pairs are ordered correctly. Together with the human result, this shows that the comparison remains learnable in both species but is harder for unseen peptides.

\subsection{Research Question 6: How Much Signal Remains without Tissue Identity?}

Removing tissue identity while retaining peptide and MHC information lowers performance in every evaluation. Under peptide-separated cross-validation, human AUROC falls from 0.7652 to 0.6630 and mouse AUROC from 0.7529 to 0.6412. The decrease occurs in 156 of 157 human combinations and all 24 mouse combinations. General peptide--MHC compatibility therefore does not reproduce the complete ranking signal, although this does not prove a causal tissue-processing mechanism.
...
\begin{table*}[t]
\centering
\caption{Comparison of controls that omit tissue identity with the full tissue-conditioned model. The control retaining peptide and MHC information has approximately the same capacity as the corresponding full model. MHCflurry uses its predicted-presentation score and NetMHCpan its eluted-ligand score. External scores are predetermined, so their values are identical across the two cross-validation partitions of the same pool. Full denotes TissuePMHC in human and the prespecified mouse mixture-of-experts model averaged across five runs.}
...
\ContinuedFloat
\centering
\caption[]{Comparison of three tissue-blind controls with the full tissue-conditioned model (continued).}
\footnotesize
\renewcommand{\arraystretch}{0.9}
\setlength{\tabcolsep}{2pt}
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{4.4cm}
...
Evaluation metric & Peptide + MHC & \shortstack{MHCflurry\\presentation} & \shortstack{NetMHCpan\\eluted-ligand score} & \shortstack{\textbf{Full}\\\textbf{tissue-conditioned}} \\
\midrule
\multicolumn{5}{@{}l}{\textit{Mouse}} \\
Saved internal test: AUROC & 0.6672 & 0.6564 & 0.6893 & 0.8562 \\
Saved internal test: AUPRC & 0.7087 & 0.6436 & 0.6926 & 0.8506 \\
Saved internal test: within-pair accuracy & 0.6354 & 0.6733 & 0.6917 & 0.8425 \\
\addlinespace
Same-pool ordinary cross-validation: AUROC & 0.6819 & 0.6474 & 0.6802 & 0.8392 \\
Same-pool ordinary cross-validation: AUPRC & 0.7008 & 0.6263 & 0.6785 & 0.8316 \\
Same-pool ordinary cross-validation: within-pair accuracy & 0.6540 & 0.6565 & 0.6888 & 0.8211 \\
\addlinespace
Peptide-separated cross-validation: AUROC & 0.6412 & 0.6474 & 0.6802 & 0.7529 \\
Peptide-separated cross-validation: AUPRC & 0.6378 & 0.6263 & 0.6785 & 0.7322 \\
Peptide-separated cross-validation: within-pair accuracy & 0.6418 & 0.6565 & 0.6888 & 0.7596 \\
\bottomrule
\end{tabular}
\end{table*}

MHCflurry and NetMHCpan perform better than random on the matched pairs (Table~\ref{tab:rq6-method-comparison}), with saved-test AUROCs of 0.6564--0.6893. General binding and presentation propensity therefore contribute useful information. The full tissue-conditioned model nevertheless has the highest value for every reported metric and evaluation.

The NetMHCpan eluted-ligand score is consistently at least as strong as its binding-affinity score. The AUROC difference between eluted-ligand score and binding-affinity score is 0.0256 on the human saved internal test set and 0.0264 on the human complete training pool, compared with 0.0046 and 0.0024 in mouse. We therefore use the eluted-ligand score as the primary NetMHCpan control and report the complete binding-affinity score sensitivity analysis in Table~\ref{tab:appendix-netmhcpan-el-ba}.

The tissue-conditioned models remain higher than the strongest tissue-agnostic control in every evaluation. In human, AUROC is 0.8448 versus 0.6851 on the saved test, 0.8279 versus 0.6871 under ordinary same-pool cross-validation, and 0.7652 versus 0.6871 after peptide separation. The corresponding mouse values are 0.8562 versus 0.6893, 0.8392 versus 0.6802, and 0.7529 versus 0.6802. The peptide-separated advantages narrow to 0.0781 in human and 0.0727 in mouse. In mouse, the latter mean difference has a 95\% combination-resampling interval of 0.0028--0.1579, but the multiplicity-adjusted Wilcoxon value is \(q=0.6840\), with the tissue-conditioned model higher in 11 combinations and lower in 13. This supports a positive mean difference rather than consistent superiority across most combinations.

Cross-fitted stacking yields only marginal improvement over the tissue-conditioned score alone. The largest change is observed for human peptide-separated cross-validation with MHCflurry, from 0.7652 to 0.7693 AUROC. Saved-test changes are 0.8448 to 0.8453 in human and 0.8562 to 0.8564 in mouse; the corresponding mouse peptide-separated change is 0.7529 to 0.7534. Complete results appear in Table~\ref{tab:appendix-general-stacking}.
...
Dual branch with auxiliary supervision & \textbf{0.7590} & 0.7797 & \textbf{0.7395} & \textbf{0.7605} & 0.6464 & 0.5494 \\
Mixture-of-experts model & 0.7529 & \textbf{0.7822} & 0.7322 & 0.7596 & \textbf{0.6481} & \textbf{0.5859} \\
\bottomrule
\end{tabularx}
\end{table*}
...
The expanded mouse comparison now matches the human strict table for one-hot logistic regression, BLOSUM62 random forest, shared encoding, and the two dual-branch controls (Table~\ref{tab:mouse-strict-architectures}). The auxiliary dual branch has the highest mean AUROC (0.7590), 0.0060 above the selected mixture-of-experts model, but this difference is not consistent across combinations (14 higher and 10 lower; 95\% interval, -0.0017 to 0.0138). Auxiliary supervision improves the matched dual branch by 0.0034 AUROC and is higher in 21 of 24 combinations (95\% interval, 0.0023--0.0047). The mixture-of-experts model exceeds one-hot logistic regression by 0.0194 AUROC and is higher in 20 combinations, but differs by no more than 0.0026 in mean AUROC from the random forest, shared encoder, or unguided dual branch. Thus, peptide-separated mouse performance supports learnability and a small benefit from auxiliary supervision, not superiority of the selected mixture-of-experts architecture.

Prediction averaging materially improves the final neural estimates. Relative to the mean of separate runs, AUROC gains are +0.0248 for the human shared encoder, +0.0161 for the unguided dual branch, +0.0139 for the guided dual branch, and +0.0147 for TissuePMHC\@. Mouse gains are +0.0211 for the shared encoder, +0.0175 for the unguided dual branch, +0.0167 for the auxiliary dual branch, and +0.0318 for the mixture-of-experts model, compared with +0.0016 for the random forest and zero for one-hot logistic regression. The mixture-of-experts mean across separate runs is only 0.7211, below the auxiliary dual branch at 0.7422 and random forest at 0.7491. Its competitive peptide-separated result is therefore driven primarily by averaging five runs rather than a consistent architectural advantage.
