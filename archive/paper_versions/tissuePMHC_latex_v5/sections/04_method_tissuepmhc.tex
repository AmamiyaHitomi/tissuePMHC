...
\subsection{Mouse Model with Combination-specific Mixtures of Shared Experts}

The mouse analysis tests whether the same prediction problem is learnable in a second species; the human model is not transferred to mouse. A separate mixture-of-experts model samples each tissue--H2 combination equally and learns a combination-specific weighting of three shared peptide-processing modules. Each residue is represented by 16 values, the complete 9-mer is encoded as a 128-dimensional vector, and each shared module produces 64 features. The weighted representation is sent to one of 24 combination-specific outputs. The model contains 68,475 trainable parameters.

Because the mouse combinations differ in size, each update draws 16 rows from every combination so that large tissues do not dominate learning. Training uses 25 epochs and AdamW optimization, and predictions are averaged across five independent runs. An additional comparison introduces a small representation adapter for H2-Kk; this option was selected before the saved internal test set was examined.

The human TissuePMHC model and the mouse mixture-of-experts model are species-specific, and no parameters are transferred between them. The comparison tests whether the benchmark question is reproducibly learnable, not whether the models share a representation or biological mechanism.
