...
Figure~\ref{fig:tissuepmhc-architecture} summarizes the two sharing pathways and their fixed-test score combination.
...
\node[widebox,fill=green!8] (ensemble) at (0,-6.75) {Three independently trained models\\summarized for stability};
\draw[arr] (pep) -- (encg);
\draw[arr] (pep) -- (ench);
...
\caption{Structural overview of the Human TissuePMHC model. The global pathway learns peptide patterns across all tissue--HLA combinations, whereas the HLA-specific pathway shares information only among tissues with the same allele. Tissue and allele prediction provide training supervision but do not enter the final score. Each training run combines the two within-combination rankings; three seeds quantify optimization stability. The Mouse implementation follows the same two-pathway principle with H2-specific sharing but is trained and tuned independently; the diagram is architectural and contains no performance result.}
...
The mouse analysis tests whether the same prediction problem is learnable in a second species; no human parameters are transferred to mouse. Mouse implementations are trained independently on the 11 retained tissue--H2 combinations and include simple sequence baselines, a shared encoder, H2 identifier and 34-residue pseudo-sequence conditioning, an identifier--pseudo-sequence hybrid, and matched dual-branch models with and without auxiliary tissue and H2 supervision. The selected Mouse TissuePMHC configuration uses convolution widths 3 and 5, dropout 0.35, tissue and H2 auxiliary-loss weights 0.05 and 0.10, equal rank-fusion weights, a learning rate of $3\times10^{-3}$, and task-balanced sampling with 32 rows drawn per task in each balanced batch. It is trained for 25 epochs. The four H2 pseudo-sequences are taken from the versioned NetMHCpan 4.1b MHC resource \cite{reynisson2020netmhcpan} and mapped from its \texttt{H-2-*} aliases to the benchmark's \texttt{H2-*} identifiers; the source-file hash is retained with the derived table. All mouse methods use the same fixed-split protocol and the same three seeds as the human experiments.
