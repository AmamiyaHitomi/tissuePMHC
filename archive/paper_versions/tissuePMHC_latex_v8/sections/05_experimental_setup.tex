...
\subsection{Post-hoc SHAP Attribution Protocol}
...
\,d\alpha\right].
\]
This is an empirical-baseline SHAP approximation for differentiable networks rather than an exact enumeration of Shapley coalitions \cite{lundberg2017shap,sundararajan2017integrated,erion2021expected}.
...
The accompanying repository contains processed benchmark tables, fixed-split row-level predictions, seed-specific configuration files, timing records, and analysis scripts. The Human interpretation release additionally records the explained pair identities, empirical-background design, per-seed and per-branch residue attributions, exhaustive ISM substitution effects, replacement rank-fusion scores, SHAP--ISM concordance, anchor-position tests, prediction-reproduction checks, seed-stability summaries, and plotting inputs. Raw material derived from the IEDB and third-party predictor assets remains subject to original access and licensing terms; NetMHCpan executables are not redistributed.
