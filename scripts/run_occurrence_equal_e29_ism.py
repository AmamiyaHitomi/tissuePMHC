...
def write_report(
    ...
    lines = [
        "# occurrence-equal tissuePMHC：E29 ISM扰动验证报告",
        "",
        "## 分析设计",
        "",
        "- 模型：正式三seed E29检查点，包括global-auxiliary与HLA-specific分支。",
        f"- 样本：每个tissue–HLA任务抽取{cli.pairs_per_task}个完整正负pair，共{sample_position['sample_id'].nunique()}条肽。",
        "- 扰动：每条9-mer的每个位点穷举替换为其余19种标准氨基酸。",
        f"- 总突变评分数：{len(effect):,}个三seed平均的单氨基酸替换。",
        "- 效应尺度：分支logit变化，以及用固定测试参考分布重新计算的最终task内百分位rank-fusion变化。",
        "- Δ定义：突变体分数减原始肽分数；负值表示突变降低模型预测。",
        "",
        "## 质量控制",
        "",
        f"- 原始肽分支概率与保存预测的最大绝对误差：{audits[['max_abs_original_probability_error_global', 'max_abs_original_probability_error_hla']].to_numpy().max():.3g}。",
        f"- 重新计算rank fusion与保存结果的最大绝对误差：{audits['max_abs_reference_fusion_error'].max():.3g}。",
        f"- 三seed位置–替换效应Spearman相关中位数：global={stability_median.get('global_logit', math.nan):.4f}，HLA={stability_median.get('hla_logit', math.nan):.4f}，fusion={stability_median.get('rank_fusion', math.nan):.4f}。",
        ...
        "### 位置敏感性",
        "",
        "- 全体样本最终融合分数最敏感的位置依次为：" + "、".join(f"P{int(p)}" for p in sensitivity_positions.index[:5]) + "。",
        "- 阳性肽中，突变最明显破坏阳性预测的位置依次为：" + "、".join(f"P{int(p)}" for p in positive_positions.index[:5]) + "。",
        f"- P2/P9平均敏感性减去其余位置为{anchor_fusion['paired_mean_difference']:.6f}，95% bootstrap CI [{anchor_fusion['bootstrap_95_ci_low']:.6f}, {anchor_fusion['bootstrap_95_ci_high']:.6f}]，单侧Wilcoxon FDR={anchor_fdr_text}。",
        "- 正负配对分析中，阳性肽分类支持损失最大的三个位置为：" + "、".join(f"P{int(row.position)}" for row in paired_top.itertuples()) + "。",
        "",
        "### 与Expected-IG/SHAP的一致性",
        "",
        f"- 逐样本逐位置的ISM平均突变损失与Expected-IG observed-residue attribution相关中位数：global={concordance_overall.get('global_aux', math.nan):.4f}，HLA={concordance_overall.get('hla_plain', math.nan):.4f}。",
        "- 两者衡量对象不完全相同：Expected-IG相对于任务背景归因，ISM比较原始肽与所有单点突变体；因此中高相关构成独立扰动证据，但不要求数值相等。",
        "",
        "## 最强降分突变（最终rank fusion）",
        "",
        "|样本|原始肽|位置|替换|Δ rank fusion|组织|HLA|",
        "|---|---|---:|:---:|---:|---|---|",
    ]
    ...
    lines.extend([
        ...
        "## 解释边界",
        "",
        "1. ISM证明的是模型对单点替换的计算依赖，不等同于体内因果效应。",
        "2. 突变后的肽可能不在训练分布内；极端效应应结合已知HLA motif、结合实验或外部预测器复核。",
        "3. 最终rank效应以原测试任务分数作为固定参考，并将原样本替换为突变体；它不是重新对整个测试集逐一联合突变。",
        "4. P2/P9是跨HLA汇总的经典锚定位置假设，等位基因特异模式应以HLA分层结果为准。",
        "5. 本分析沿用SHAP相同的每任务抽样测试pair；结论代表所解释样本，不应外推为全部可能肽空间。",
        "",
    ])
    (cli.output_dir / "ISM_ANALYSIS_REPORT_zh.md").write_text("\n".join(lines), encoding="utf-8")
