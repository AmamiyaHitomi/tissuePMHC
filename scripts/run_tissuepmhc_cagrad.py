#!/usr/bin/env python3
"""Run E9 CAGrad on the E2 shared-head performance line for tissuePMHC.

Roadmap role: E9-on-E2 performance line.
This is the official CAGrad experiment after inserting:

    E6: task grouping on E2
    E7: validation-based selective HLA/global sharing
    E8: soft ensemble of global and HLA branches

CAGrad means Conflict-Averse Gradient Descent. Here it is applied to the E2
architecture:
...
2. E9_E2_CAGrad: conflict-averse gradient mixing across tasks.

This file intentionally does not use HLA pseudo-sequences. The E4
pseudo-sequence line is a separate biological-representation branch.
"""
...
MODEL_CHOICES = ["e2_task_balanced", "e9_e2_cagrad"]
...
def flatten_gradients(torch: Any, gradients: tuple[Any, ...], parameters: list[Any]) -> Any:
    flattened = []
...
def write_flat_gradient_to_parameters(flat_gradient: Any, parameters: list[Any]) -> None:
    offset = 0
    for parameter in parameters:
        n_values = parameter.numel()
        parameter.grad = flat_gradient[offset : offset + n_values].view_as(parameter).clone()
        offset += n_values


def project_to_simplex(torch: Any, values: Any) -> Any:
    sorted_values, _ = torch.sort(values, descending=True)
    cumulative = torch.cumsum(sorted_values, dim=0) - 1.0
    positions = torch.arange(1, values.numel() + 1, device=values.device, dtype=values.dtype)
    support = sorted_values - cumulative / positions > 0
    rho = torch.nonzero(support, as_tuple=False)[-1, 0]
    theta = cumulative[rho] / (rho.to(values.dtype) + 1.0)
    return torch.clamp(values - theta, min=0.0)


def min_norm_simplex_weights(torch: Any, gradients: Any, steps: int, lr: float) -> Any:
    n_tasks = gradients.shape[0]
    weights = torch.full((n_tasks,), 1.0 / n_tasks, device=gradients.device, dtype=gradients.dtype)
    gram = gradients @ gradients.t()
    for _ in range(steps):
        weights = project_to_simplex(torch, weights - lr * (gram @ weights))
    return weights


def pairwise_gradient_stats(torch: Any, gradients: Any) -> dict[str, float]:
    normalized = gradients / (gradients.norm(dim=1, keepdim=True) + 1e-12)
    cosine = normalized @ normalized.t()
    n_tasks = cosine.shape[0]
    mask = ~torch.eye(n_tasks, dtype=torch.bool, device=cosine.device)
    pairwise = cosine[mask]
    return {
        "mean_pairwise_gradient_cosine": float(pairwise.mean().detach().cpu()),
        "min_pairwise_gradient_cosine": float(pairwise.min().detach().cpu()),
        "negative_pair_fraction": float((pairwise < 0).float().mean().detach().cpu()),
    }
...
def train_one_model(
    ...
    use_cagrad = model_name == "e9_e2_cagrad"
    experiment_name = "E9_E2_CAGrad" if use_cagrad else "E2_task_balanced"
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for _ in range(steps_per_epoch):
            ...
            if use_cagrad:
                losses = [task_loss(torch, model, batch, device) for batch in batches]
                gradients = [
                    flatten_gradients(
                        torch,
                ...
                task_gradients = torch.stack(gradients, dim=0)
                mean_gradient = task_gradients.mean(dim=0)
                weights = min_norm_simplex_weights(
                    torch,
                    task_gradients.detach(),
                    args.cagrad_solver_steps,
                    args.cagrad_solver_lr,
                )
                min_norm_gradient = weights @ task_gradients
                combined_gradient = (1.0 - args.cagrad_alpha) * mean_gradient + args.cagrad_alpha * min_norm_gradient
                if args.max_grad_norm > 0:
                    gradient_norm = combined_gradient.norm()
                    if gradient_norm > args.max_grad_norm:
                        combined_gradient = combined_gradient * (args.max_grad_norm / (gradient_norm + 1e-12))
                write_flat_gradient_to_parameters(combined_gradient, parameters)
                optimizer.step()
                epoch_losses.append([float(loss.detach().cpu()) for loss in losses])
                epoch_weights.append(weights.detach().cpu().numpy())
                epoch_stats.append(pairwise_gradient_stats(torch, task_gradients.detach()))
            else:
                losses = [task_loss(torch, model, batch, device) for batch in batches]
                ...
                optimizer.step()
                epoch_losses.append([float(loss.detach().cpu()) for loss in losses])
                epoch_weights.append(np.full(len(task_arrays), 1.0 / len(task_arrays), dtype=np.float64))

        mean_losses = np.asarray(epoch_losses, dtype=np.float64).mean(axis=0)
        mean_weights = np.asarray(epoch_weights, dtype=np.float64).mean(axis=0)
            ...
            weight_rows.append(
                {
                    ...
                    "cagrad_weight": float(mean_weights[task_index]),
                    "epoch_mean_loss": float(mean_losses[task_index]),
                }
            )

        if epoch_stats:
            mean_pairwise = float(np.mean([row["mean_pairwise_gradient_cosine"] for row in epoch_stats]))
            min_pairwise = float(np.min([row["min_pairwise_gradient_cosine"] for row in epoch_stats]))
            negative_fraction = float(np.mean([row["negative_pair_fraction"] for row in epoch_stats]))
        else:
        ...
        diagnostic_rows.append(
            {
                ...
                "mean_pairwise_gradient_cosine": mean_pairwise,
                "min_pairwise_gradient_cosine": min_pairwise,
                "negative_pair_fraction": negative_fraction,
                "mean_cagrad_entropy": float(np.mean([entropy(weights) for weights in epoch_weights])),
                "min_cagrad_weight": float(np.min(mean_weights)),
                "max_cagrad_weight": float(np.max(mean_weights)),
            }
        )
        print(
            ...
            f"min_weight={float(np.min(mean_weights)):.4f} max_weight={float(np.max(mean_weights)):.4f}"
        )
...
def run_one_experiment(
    ...
    set_seed(seed, torch)
    experiment_name = "E9_E2_CAGrad" if model_name == "e9_e2_cagrad" else "E2_task_balanced"
...
def run(args: argparse.Namespace) -> None:
    ...
    metadata = {
        ...
        "cagrad_solver_steps": args.cagrad_solver_steps,
        "cagrad_solver_lr": args.cagrad_solver_lr,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--summary-output",
        type=Path,
        default=project_path("results/tissuePMHC_cagrad/summary_metrics.csv"),
    )
    parser.add_argument(
        "--metadata-output",
        type=Path,
        default=project_path("results/tissuePMHC_cagrad/metadata.json"),
    )
    parser.add_argument(
        "--stability-output",
        type=Path,
        default=project_path("results/tissuePMHC_cagrad/stability_metrics.csv"),
    )
    parser.add_argument(
        "--comparison-output",
        type=Path,
        default=project_path("results/tissuePMHC_cagrad/comparison_metrics.csv"),
    )
    parser.add_argument(
        "--external-comparison-output",
        type=Path,
        default=project_path("results/tissuePMHC_cagrad/external_comparison_metrics.csv"),
    )
    parser.add_argument(
        "--weight-output",
        type=Path,
        default=project_path("results/tissuePMHC_cagrad/cagrad_weight_history.csv"),
    )
    parser.add_argument(
        "--diagnostic-output",
        type=Path,
        default=project_path("results/tissuePMHC_cagrad/gradient_diagnostics.csv"),
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.5,
        help="Interpolation strength from mean gradient to conflict-averse min-norm gradient.",
    )
    parser.add_argument("--cagrad-solver-steps", type=int, default=25)
    parser.add_argument("--cagrad-solver-lr", type=float, default=0.1)
